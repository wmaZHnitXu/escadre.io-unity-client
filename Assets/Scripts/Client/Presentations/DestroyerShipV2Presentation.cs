// File: Scripts/Client/Presentations/DestroyerShipV2Presentation.cs
...
public class DestroyerShipV2Presentation : ClientProxyPresentation
{
    [Header("DestroyerShipV2Presentation Visuals")]
    ...
    protected override void OnInitialized()
    {
        ...
        if (_shipProxy == null)
        {
            Logger.LogError($"[DestroyerShipV2Presentation {gameObject.name}] TargetProxy is not a ShipProxy.ClientProxy! Type: {TargetProxy?.GetType().Name}");
            enabled = false;
            return;
        }
        ...
        {
            Logger.LogWarning($"[DestroyerShipV2Presentation {gameObject.name}] ClientComposer not found in scene. Cannot determine advanced tap area states.");
        }
