// File: Scripts/Client/Presentations/DestroyerShipPresentation.cs
...
public class DestroyerShipPresentation : ClientProxyPresentation
{
    [Header("DestroyerShipPresentation Visuals")]
    ...
    protected override void OnInitialized()
    {
        ...
        if (_shipProxy == null)
        {
            Logger.LogError($"[DestroyerShipPresentation {gameObject.name}] TargetProxy is not a ShipProxy.ClientProxy! Type: {TargetProxy?.GetType().Name}");
            enabled = false;
            return;
        }
        ...
        {
            Logger.LogWarning($"[DestroyerShipPresentation {gameObject.name}] ClientComposer not found in scene. Cannot determine advanced tap area states.");
        }
